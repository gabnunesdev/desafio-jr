      ...
      <RadioGroup value={value} onValueChange={onChange} className="flex gap-4">
        {PET_OPTIONS.map((option) => (
          <div key={option.value} className="flex-1">
            <RadioGroupItem value={option.value} id={option.id} className="peer sr-only" disabled />
            <Label
              htmlFor={option.id}
              className={`
                flex items-center justify-start px-4 gap-3 h-12 rounded-xl border transition-all cursor-default
                ${
                  value === option.value
                    ? "bg-transparent text-white border-white border-2 font-bold"
                    : "bg-[#0F1629]/50 text-gray-500 border-slate-700/50"
                }
              `}
            >
              <div
